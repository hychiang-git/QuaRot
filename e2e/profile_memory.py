...
def benchmark(args):
    ...
    for config_name in model_configs:
        ...
        buffer_size = 0
        for buffer in model.model.embed_tokens.buffers():
            buffer_size += buffer.nelement() * buffer.element_size()
        model_mb = (param_size + buffer_size) / 1024**2
        print('embed_tokens size: {:.3f} MB'.format(model_mb))
        ...
        for i in range(n_layers):
            ...
            for param in model.model.layers[i].parameters():
                param_size += param.nelement() * param.element_size()
            buffer_size = 0
            for buffer in model.model.layers[i].buffers():
                print(buffer.shape)
                buffer_size += buffer.nelement() * buffer.element_size()
            model_mb = (param_size + buffer_size) / 1024**2
            print('layer {} size: {:.3f} MB'.format(i, model_mb))
        param_size = 0
        ...
        for buffer in model.model.rotary_emb.buffers():
            buffer_size += buffer.nelement() * buffer.element_size()
        model_mb = (param_size + buffer_size) / 1024**2
        print('rotary_emb size: {:.3f} MB'.format(model_mb))
        param_size = 0
        for param in model.lm_head.parameters():
            param_size += param.nelement() * param.element_size()
        buffer_size = 0
        for buffer in model.lm_head.buffers():
            buffer_size += buffer.nelement() * buffer.element_size()
        model_mb = (param_size + buffer_size) / 1024**2
        print('lm_head size: {:.3f} MB'.format(model_mb))
        print(model.lm_head.weight.data_ptr() == model.model.embed_tokens.weight.data_ptr()) # prints True
